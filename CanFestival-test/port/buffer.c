...
void buffer_init(u8 buffer[], u8 size)
{
	buffer[0] = size;
	buffer[1] = 0;		/* Head */
	buffer[2] = 0;		/* Tail */
...
bool buffer_output_free(u8 buffer[])
{
    u8 tmphead  = (buffer[1] + 1);
	if (tmphead == buffer[0]) tmphead = 0;
...
bool buffer_input_available(u8 buffer[])
{
